...
#@(#)   v0.26   11feb2023: use hw accell cuda instead of cuvid leaving output to cuda (not auto) and change preset to p7 -tune hq and 10 bit p010le for hvec
###################################################################################################################################
...
then
    ...
    do
        ...
        then
            ...
            #
            mkvtitle=`echo "${mkv_lines}" | grep -i "title" | head -1  |awk 'NR==0; END{print}' | sed "s/title\s*:\s//"`
            ...
            case $k in
                    4)
                        encoder="-c:V h264_nvenc -preset:V p7 -tune hq -rc cbr -profile:V high -rc-lookahead 20"
                        tagenc="nvidia264"
                        ;;
                    5)
                        encoder="-vf scale_cuda=format=p010le -c:V hevc_nvenc -preset:V p7 -tune hq -rc cbr -profile:V main10 -rc-lookahead 20"
                        tagenc="nvidia265"
                        ;;
                     *)
