...
then
    ...
    do
        ...
        then
            ...
            case $k in
                    4)
                        encoder="-c:V h264_nvenc -preset:V p7 -tune hq -profile:V high -rc-lookahead:v 32 -spatial_aq 1 -cq 29"
                        tagenc="nvidia264"
                        ;;
                    5)
                        encoder="-vf scale_cuda=format=p010le -c:V hevc_nvenc -preset:V p7 -tune hq -profile:V main10 -rc-lookahead:v 32 -spatial_aq 1 -rc=vbr -cq 20 -qmin=20 -qmax=25"
                        tagenc="nvidia265"
                        ;;
                     *)
